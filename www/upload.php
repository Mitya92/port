...
<?	
	echo '<div class="ind"><div class="comm">';
		...
		$result = mysql_query("select * from upload  where `types` = 0 and `visible` = 1 order by id desc limit 50 ;");
		while ($row = mysql_fetch_array($result)) echo '<div class="com"><a href ="./files/img/'.$row[name].'">'.$row[name].'</a></div>';
		echo '<b>Остальные файлы:</b>';
		$result = mysql_query("select * from upload  where `types` = 1 and `visible` = 1 order by id desc limit 50 ;");
		while ($row = mysql_fetch_array($result)) echo '<div class="com"><a href ="./files/upload/'.$row[name].'">'.$row[name].'</a></div>';
	echo '</div></div>';
